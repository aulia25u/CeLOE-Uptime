 ...
 */
console.log("Welcome to CeLOE Uptime");
...
// Exit CeLOE Uptime immediately if the Node.js version is banned
...
log.info("server", "CeLOE Uptime Version: " + checkVersion.version);
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error("CeLOE Uptime has been initialized. If you want to run setup again, please delete the database.");
                }
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new CeLOE Uptime instance, need to setup
    if ((await R.knex("user").count("id as count").first()).count === 0) {
